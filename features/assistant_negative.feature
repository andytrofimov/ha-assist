...
Функция: Негативные сценарии ассистента умного дома
  ...
  @skip
  Сценарий: Числительное в яркости распознается как процент
    Когда пользователь говорит "включи свет в гостиной на пятнадцать процентов"
    Тогда ассистент вызывает сервисы:
      | domain | service | entity_id             | brightness_pct |
      | light  | turn_on | light.svet_gostinnaia | 15             |

  @skip
  Сценарий: Числительное в задержке распознается как минуты
    Когда пользователь говорит "выключи свет в гостиной через пятнадцать минут"
    Тогда ассистент вызывает сервисы:
