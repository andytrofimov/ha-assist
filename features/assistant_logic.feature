...
Функция: Действия ассистента умного дома
  ...
  Сценарий: Уточнить комнату если bare-обращение совпадает с несколькими сценами
    ...
    И доступны комнаты:
      ...
      | spalnia   | Спальня  | vtoroi   |         |
    Когда пользователь говорит "сцена кино"
    Тогда ответ ассистента равен "В какой комнате?"
  ...
  Сценарий: Составная команда с отложенным действием и обычным действием
    Когда пользователь говорит "включи свет в кабинете через десять секунд и закрой ворота"
    Тогда ассистент вызывает сервисы:
