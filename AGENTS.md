...
- `main.py` - FastAPI-приложение. В нем должны оставаться только `/assist` и `/health`.
- `assistant_logic.py` - основная логика разбора команд умного дома.
- `api_models.py` - модели request/response для `/assist`.
- `ha_parser.py` - модель `HaObject`.
- `openai_compat.py` - вынесенная заготовка OpenAI-compatible логики, сейчас не зарегистрирована как эндпоинт.
...
`assistant_logic.py` поддерживает:
