...
- приоритет выбора: bare-activation scene/button по полному совпадению имени или alias идет перед всем остальным, затем
  конкретная сущность по имени или alias, и только потом broad-команды по домену вроде `выключи свет`;
